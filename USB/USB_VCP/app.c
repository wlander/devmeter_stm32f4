...
/**
  ...
  * @retval None
  */
int USB_VCP_Init (void)
{
	uint16_t prov=0;
	uint8_t Buf_Temp[3] = {0x31,0x32,0x33};
...
VCP_DataTx (Buf_Temp, 3);			// Функция считывает данные с COM-порта и передает их по USB (если COM не подключен, то передает НУЛИ)!!!
