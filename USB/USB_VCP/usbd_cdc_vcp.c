...
extern FIFO( ERROR_FIFO_SIZE ) Error_fifo;
...
{
//	APP_Rx_Buffer[0] = 0x31;
//	APP_Rx_Buffer[1] = 0x32;
//	APP_Rx_Buffer[2] = 0x33;

/*  if (linecoding.datatype == 7) {
    APP_Rx_Buffer[APP_Rx_ptr_in] = USART_ReceiveData(EVAL_COM1) & 0x7F;
  } else if (linecoding.datatype == 8) {
    APP_Rx_Buffer[APP_Rx_ptr_in] = USART_ReceiveData(EVAL_COM1);		// Считываем с компорта и передаем по USB !!!
//    APP_Rx_Buffer[APP_Rx_ptr_in] = 0x31;		// Передаем "1" 
    APP_Rx_Buffer[APP_Rx_ptr_in] = Buf[0];		// Передаем "1" 
  }
 */
	APP_Rx_Buffer[APP_Rx_ptr_in] = Buf[0];		// Передаем "1" 
...
void VCP_DataTx_ (uint8_t* Buf, uint32_t Len){
	int i = 0;

  for(i=0; i<Len; i++)
	{
	  APP_Rx_Buffer[APP_Rx_ptr_in] = Buf[i];			// Передаем 1 байт
		APP_Rx_ptr_in++;
 ...
 void OUT_DataTx_USB (void)
{
	int i = 0;
//	uint8_t rxbyte[1];
  for(i=0; ((!(FIFO_IS_EMPTY( MK_to_Comp_fifo ))) && (i<APP_RX_DATA_SIZE)); i++)
	{
		//если данных в fifo больше нет то запрещаем это прерывание
		WIM_task &= ~WIM_MK_TO_COMP;

		//передаем байт
	  APP_Rx_Buffer[APP_Rx_ptr_in] = FIFO_FRONT( MK_to_Comp_fifo );			// Передаем 1 байт
		APP_Rx_ptr_in++;
	  FIFO_POP( MK_to_Comp_fifo );
		if (APP_Rx_ptr_in == APP_RX_DATA_SIZE) 
		{
			APP_Rx_ptr_in = 0;
		}  
  ...
  for(; !(FIFO_IS_EMPTY( MK_to_Comp_fifo )); )
	{
		...
		//передаем байт
	  rxbyte [0] = FIFO_FRONT( MK_to_Comp_fifo );
	  FIFO_POP( MK_to_Comp_fifo );

//		while(!(USART1->SR & USART_SR_TC));
//		USART1->DR=rxbyte;										// Передача байта

//		for (i=0; i<16; i++)
//		{
//		rxbyte[0] = (uint8_t) i;
//		VCP_DataTx (rxbyte, 1);			// Передача байта
//		}
...
void OUT_DataTx_USB_Error (void)
{
	uint8_t rxbyte[1];
  for(; !(FIFO_IS_EMPTY( Error_fifo )); )
	{
		//если данных в fifo больше нет то запрещаем это прерывание
	  WIM_task &= ~WIM_FIFO_FILLED;			// Сбрасываем флаг	

		//передаем байт
	  rxbyte [0] = FIFO_FRONT( Error_fifo );
	  FIFO_POP( Error_fifo );
